...
**Answer**
This is a table of voter registration but it has some issues. The numbers are flushed left, meaning R recognizes them as a character. A lot of the columns are also separated with spaces and do not have labels.
```{r}
...
**Answer** This is a table of voter registration. This table is much cleaner with each party as a column and the registration method as another. The numbers are no longer characters. 
...
**Task** Run the following code and describe the result.
**Answer** This is a table of just removals. There is a column with for each party and the registration method as another. The numbers are also no longer characters. 
...
**Answer**
I think it is interesting that the number of unaffiliated new registrants is higher than Republican new registrants in most methods. It it not surprising for Democrats to be higher than Republicans since the state is left leaning. However, I think the rise is unaffiliated voters could mean new voters are not aligning with a specific party. 
```{r}
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer** I think it is interesting that the number of criminal conviction removals for unaffiliated is at 26%. This method is the highest percent for unaffiliated and is more than double the Republican percentage. I also think it is interesting that the voter request method is higher for unaffiliated than it is for Republicans. 
...
**Answer** 
There are 16 rows and 25 columns for oct_jun most of the other tables have 17 rows.
...
**Task** Run the following code. How many rows do you have compared to either the October or June dataframes?
**Answer** There are now 17 rows, which is the same as the other dataframes. 
...
**Answer** The change is rather small for each column. One of the biggest differences was for Republicans in provisional ballots which was 17%. 
...
**Answer**The trend is that ERIC mail registrations are mostly Democrat and is higher than any other method for the party. 
...
**Answer** Eric Report was higher in October than it was in June. Provisional ballot was the top method in June but Eric was fifth this month. In person also became more popular from October to June.
