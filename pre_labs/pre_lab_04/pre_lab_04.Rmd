...
**Answer** 24 rows and there should be 24 because the amount row was left off. 
...
**Answer** I did not get an error message. This is now a more informed dataset that includes demographics.

```{r}
# join on zip - but there's a problem
#remeber that the first column in a join comes from the first dataframe in the statement
...
**Answer** County with the highest 18+ Black population is Prince George's county at 61% population. 78% are democrats. 
...
**Answer** Howard County has the highest 18 Asian plus population at 19%. Of the 18 plus population 52% are democrats.
...
**Answer** The datatype is DBL or a number. 
```{r}
...
**Answer** ZCTA5N is now a character instead of a number. 
