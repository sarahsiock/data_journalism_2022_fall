...
**Answer**There is an problem in each row. R is expecting a true or false value but that is not what it is finding. It currently cannot read dates or characters.
...
**Answer** I do not see any errors. The problem message no longer exists. 
...
**Answer** District:DBL (number) Precinct:CHR (character) Mail:DBL (number)
```{r}
...
**Answer**:There are 402,345 results where mail is NA.This implies those votes are not accounted for in the mail in category. Maybe counties didn’t report votes by mail as a separate category or called it something else.
```{r}
...
**Answer**There are 12 registries from January 1900. The first 20 results suggest that there were errors when filing this data. It is hard to believe that were 12 registered voters born in 1900. Therefore, some of this information would have to be eliminated when looking at the data moving forward.
```{r}
yadkin_voters %>% 
...
**Answer**Those that were removed for being inactive voters could make an interesting story. I would be curious to find out why these people were not voting and what would make them motivated to vote.
...
**Answer**
-The column headers are inconsistent. The first column “1_linenumber” starts with a number. The “LAST_NAME” column is all caps, while the rest are lowercase. "Address one” has a space in it. 
-The amount column is stored as a character instead of a number, which will prevent caculations.
-The street address column is inconsistent. It has multiple variations of Merry Knoll Lane.
...
**Answer**It put an x in front of 1_linenumber.Additionally, last_name is now lowercase and there is no longer differentiation among the addresses. 
...
I think this means there is an error because the amount column is stored as a character data type and not a number. Therefore, it cannot be used for any calculations. 
```{r}
# cleaning function
...
**Answer** There are now only zip codes with five digits.
...
Conowingo is still spelled wrong twice. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer** It is still spelled wrong once. 
...
**Answer**There are no misspelling. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
