...
```{r}
voters_by_county <- read_csv("tabula-Eligible Active Voters by County - GP22-2.csv")
...
**Answer** The last row SUNRISE BEACH 2 & 3 AND WILSON BEND was split up into two rows. The absentee column also has dashes instead of N/A. Additionally,jurisdiction wide which is a header, is in a row. 
...
**Answer**
Jurisdiction Wide is still a row instead of column and bend is still its own column. 
...
**Answer** It is now 21 rows and 4 columns. Compared to 23 rows. 
