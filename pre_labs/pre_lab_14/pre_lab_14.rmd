...
**Answer** This is a dataframe with 999 rows and four columns. There is a date, title, url and text column.
...
**Answer** Unique words shows how one word can be displayed differently throughout the dataframe.
...
**Answer** The output is a list of all the words in the press releases.
...
**Answer** This is not very interesting because the most common words are very simple words like "the" and "of."
...
**Answer**
This is much more interesting because the simple words are removed. The words actually give context to what Cardin was writing about in the press releases. 
```{r}
unique_words %>%
...
**Answer** This is a list of the top 10 words in each year. One difference between the two years is that in 2022 COVID is not in the top 10. 
...
**Task** Run the following code and describe the results.
**Answer** This is a list of two words that appeared frequently together in 2021. Covid 19 is the most common phrase. 
...
**Task** Run the following code and describe the results.
**Answer**
This is a list of two words that appeared frequently together in 2022. Covid 19 is the most common phrase. 
```{r}
releases %>%
  filter(date >= '2022-01-01') %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
...
**Answer**
There are a few words that are not always positive or negative. Some examples include free, work and job. However, in general the sentiments seems correct. 
```{r}
