...
**Answer** First arrange in descending order by the column total difference. Select to show only county and total difference and then display first 10 rows.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**A bar graph with counties on X axis and total difference on the Y axis.
...
**Answer** the verb reorder was added which then placed the counties from least to greatest amount.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**Now the counties are on the Y axis and count is the X axis. This makes the graph much easier to read because the county names no longer overlap
...
**Answer** This graph is using the economist them which adds specific colors/fonts.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The graph now has a title. The X and Y axis are also labeled and a caption is on the bottom now. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**A tibble with a column for dates and another for total contributions.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**A line graph with total contributions on the Y axis and date on the X axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now the  X axis lists the dates by week and they are labeled with each week.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now the dates are slightly slanted, making them easier to read.
```{r}
md_winred_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
