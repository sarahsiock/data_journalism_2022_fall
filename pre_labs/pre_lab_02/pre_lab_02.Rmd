...
**Answer** The mutate function adds a new column to the data set. In this case, a column was added to show the percent of those who voted in person on election day. Additionally, the function "select" also limits the table to only have certain columns. In this case it only includes office, district, name_raw, party, jurisdiction, election_day and votes. 
...
**Answer** The select and mutate functions stayed the same, but the arrange function was added. Now, it says arrange(desc(percent_election_day)) in the last line. This puts the percents for election day voting from least to most.
...
**Answer** By organizing the data this way, you can see which candidates received the most in-person votes. By doing this function you are able to see which candidates can count on in-person votes and make predictions about future elections.For example, people who vote in-person may change their vote the day of, while those who vote by mail are less likely to change their vote. 
...
**Answer**
str_to_lower: standardizes by lower case letter.
str_to_title: used to convert the first letter of every word of a string to Uppercase and the rest of the letters are converted to lower case.
...
**Answer** The code block added a column for "amount category." The column will categorize a contribution by its amount. First, the contributions less than $100. Then the contributions that are between $100 and $500. The next line is those between $500 and $2900. Finally, contributions less than $2,900. 
...
**Answer** A new variable allows you to save the data set just for Prince George's county. This is important if you want to use this variable in the future to find specific information about the county. Now you can use the group by function and others just for Prince George's county. 
...
**Answer:**Now, the column says <date> for the character type. Also, the date is written in year,month,day format. 
...
**Answer**The data is organized by the total amount of contributions for each month. The values in the month column reflect the first day of each month. However, the data shows total contributions for the entire month. 
