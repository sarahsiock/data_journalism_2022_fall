...
```{r}
maryland_winred_contributions <- read_rds("~/Documents/GitHub/data_journalism_2022_fall/pre_labs/pre_lab_01/pre_lab_01.rds")
```
...
**Answer** (chr)and (dbl). I'm not really sure what they stand for. Maybe an abbreviation for how they are coded?
...
**Answer** Beth Hansher (7205 Arrowood Rd)
...
**Answer** It is sorting contribution amounts by dates.
...
**Answer** The contributions are listed from most to least in a day.
...
Compared to the previous codeblock, what has changed and how did that affect the result?
**Answer** The group by function changed. It is sorted by how many people donated a certain dollar amount to a specific organization. It is also arranged by most frequent to least frequent. 
...
```
Compared to the previous codeblock, what has changed and how did that affect the result?
**Answer** It is reflecting the total dollars raised for each campaign instead of individual dollar amount.
...
**Answer**This table is listing the number of contributions for each organization and the total amount raised. A mean and median is also included for each organization. 
...
**Answer**
Now the data is grouped by city with the number of contributions from each city and the total amount of those contributions. There is also a mean and median listed for each city.
